...
class Grid(object):
    ...
    def create_from_str(grid_class, grid_str, figsize=None):
        ...
        lines = list(map(str.split, filter(lambda s:not s.startswith('#') and len(s)>0, list(map(str.strip,grid_str.split('\n'))))))
        num_rows = len(lines)
    ...
    def get_cells_of_type(self, what_type):
        ...
        return list(zip(*np.where(self.grid_array == what_type)))
